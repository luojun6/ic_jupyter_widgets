...
from components.Foreground import UISimpleForeground
...
class IviAvmCamPM:
    def __init__(
        ...
        self.__speed = speed
        self.__speed_close_threshold = widgets.IntText(
            value=75, description="CloseSpeed", disabled=True
        )
        self.__speed_open_threshold = widgets.Dropdown(
            options=["15", "25", "35"], value="35", description="OpenSpeed"
        )
        self.__speed_control = widgets.VBox(
            [self.__speed, self.__speed_close_threshold, self.__speed_open_threshold]
        )
        self.__control = None
        self.__display = None

        self.__speed.observe(self.on_change_speed_callback, "value")
        self.__speed_open_threshold.observe(
            self.__on_change_speed_open_threshold_callback, "value"
        )
        self.__foreground.set_on_clink_avm360_button_callback(
            self.on_click_avm360_button
        )
    ...
    def on_click_avm360_button(self, btn):
        if int(self.__speed.value) > int(self.__speed_open_threshold.value):
            self.logger.debug(
                f"{__class__.__name__} forbid to enter avm360 in speed of {self.__speed.value}."
            )
        else:
...
class IviAvmCamPM_DVR(IviAvmCamPM):
    ...
    def __init__(
        ...
        super().__init__(
            logger, foreground=foreground, avm_cam_pm=avm_cam_pm, speed=speed
        )
...
class IviAvmCamPM_MPD(IviAvmCamPM_DVR):
    ...
    def __init__(
        ...
        self.__cam_occupied_signal_control = widgets.HBox(
            [self.__select_label, self.__cam_occupied_signal_select]
        )
        ...
        self.__cam_occupied_signal_select.observe(
            self.on_change_cam_occupied_signal, "value"
        )
    ...
    def construct(self):
        top_control = widgets.VBox(
            [self.avm_cam_pm.show(), self.__cam_occupied_signal_control]
        )
        buttom_control = widgets.HBox(
            [self.speed_control, widgets.VBox([self.sd_card, self.__energy_mode])]
        )
    ...
    def on_change_energy_mode(self, change):
        ...
        if new_value == self.__energy_mode.SUPER_ECO:
            self.logger.debug(
                f"{__class__.__name__}: energy_mode changed to {self.__energy_mode.SUPER_ECO}."
            )
            if (
                self.__cam_occupied_signal_select.value
                != self.CAMERA_OCCUPIED_SIGNAL[2]
            ):
                self.logger.debug(f"{__class__.__name__}: MPD power off all cameras.")
                self.avm_cam_pm.power_off()
            else:
                self.logger.debug(
                    f"{__class__.__name__}: cameras are occupied, unable to power off cameras."
                )

        elif new_value == self.__energy_mode.NORMAL:
            self.logger.debug(
                f"{__class__.__name__}: energy_mode changed to {self.__energy_mode.NORMAL}."
            )
    ...
    def on_change_cam_occupied_signal(self, change):
        new_value = change["new"]
        if (new_value != self.CAMERA_OCCUPIED_SIGNAL[2]) and (
            self.__energy_mode.value == self.__energy_mode.SUPER_ECO
        ):
            self.avm_cam_pm.power_off()
        else:
            self.avm_cam_pm.power_on()
